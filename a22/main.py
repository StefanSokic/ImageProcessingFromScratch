...
def forwardFT( image ):
  ...
  # initialize the new array of complex numbers that we'll return
  for row in range(len(image)):
    new_image = np.zeros((len(image), len(image[row])), complex)

  # loop through each row and pass it into the ft1D() function
  for row in range(len(image)):
    new_image[row] = ft1D(image[row])

  # transpose the matrix so we can loop through the columns
  new_image = new_image.T

  # loop through each column and pass it into the ft1D() function
  for column in range(len(new_image)):
    new_image[column] = ft1D(new_image[column])
...
def inverseFT( image ):
  ...
  # get the conjugate of the image
  image = np.conj(image)
  ...
  # loop through each row and pass it into the ft1D() function
  for row in range(len(image)):
      # dividing by N (len of array) as we add it to the column
      new_image[row] = ft1D(image[row]) / len(image)
  ...
  for column in range(len(new_image)):
      # dividing by N (len of array) as we add it to the column
      new_image[column] = ft1D(new_image[column]) / len(image)

  # get the conjugate of the image again
  new_image = np.conj(new_image)
...
def multiplyFTs( image, filter ):
  # make a new filter so that we don't alter the original
  new_filter = [[0 for j in i] for i in filter]

  # loop through all the pixels in the image
  for i in range(len(image)):
      for j in range(len(image[i])):
          # (e^{-2 pi i * ((-N/2)/N)*u}) * (e^{-2 pi i * ((M/2)/M)*v}) gets reduced down to (-1) ** (u + v)
          new_filter[i][j] = filter[i][j] * ((-1) ** (i + j))

  return image * new_filter
        ...
        if not isFT:
            ...
            for i in range(radius):
                ...
                for j in range(radius):
                    ...
                    dist_i = y - row
                    dist_j = x - col
                    gauss = gaussian(dist_i, dist_j, radius)
                    print dist_i, dist_j, radius
                    image[row][col] = (image[row][col]) * (1 - gauss)
        else:
            row = y - (radius // 2) + 1
            row_s = ydim-1-row-(radius//2)+1
            for i in range(radius):
                row += 1
                row_s += 1
                col = x - (radius // 2) + 1
                col_s = xdim-1-col-(radius//2)+1
                for j in range(radius):
                    col += 1
                    col_s += 1
                    dist_i = y - row
                    dist_j = x - col
                    gauss = gaussian(dist_i, dist_j, radius)
                    image[row][col] = np.exp(np.log(image[row][col]) * (1 - gauss))
                    image[row_s][col_s] = np.exp(np.log(image[row_s][col_s]) * (1 - gauss))
